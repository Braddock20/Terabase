...
class TeraBox:
    ...
    def __init__(self, db, account_id=None):
        self.db=db; self.account_id=account_id
        self.client=httpx.AsyncClient(timeout=httpx.Timeout(settings.request_timeout, connect=30), follow_redirects=True)
        self._load()
    def _load(self):
        a=self.db.account(self.account_id) if self.account_id else None
        self.access=(a or {}).get('access_token','') if a else settings.terabox_access_token
        self.refresh=(a or {}).get('refresh_token','') if a else settings.terabox_refresh_token
        self.api_domain=(a or {}).get('api_domain','') if a else ''
        self.upload_domain=(a or {}).get('upload_domain','') if a else ''
        self.expires_at=float((a or {}).get('expires_at',0) or 0) if a else 0
        self.folder=(a or {}).get('folder','') if a else '/From: Other Applications/'+settings.terabox_app_folder.strip('/')+'-'
    ...
    async def refresh_token(self):
        ...
        d=j['data']; self.access=d['access_token']; self.refresh=d['refresh_token']; self.expires_at=time.time()+int(d.get('expires_in',172800)); self._save(); await self.token_info(force=True); return d
    def _save(self):
        if self.account_id:self.db.update_account(self.account_id,access_token=self.access,refresh_token=self.refresh,api_domain=self.api_domain,upload_domain=self.upload_domain,expires_at=self.expires_at,folder=self.folder)
    async def token_info(self,force=False):
        if not self.access: raise TeraBoxError('Connect TeraBox first')
        if not force and self.api_domain and self.upload_domain and time.time() < self.expires_at-3600:return
        j=await self._post('/oauth/tokeninfo',{'access_token':self.access}); d=j.get('data',{})
        self.api_domain=d.get('api_domain','').rstrip('/'); self.upload_domain=d.get('upload_domain','').rstrip('/'); self.expires_at=time.time()+int(d.get('expires_in',172800)); self._save()
    ...
    def safe_path(self,p): return self.folder.rstrip('/')+'/'+p.lstrip('/')
    async def quota(self):
        await self.ensure(); r=await self.client.get(self.q('/openapi/api/quota')); r.raise_for_status(); j=r.json();
        if j.get('errno',0)!=0: raise TeraBoxError(j.get('show_msg') or str(j)); return j.get('data',j)
    ...
    async def upload_shard(self,path,uploadid,partseq,data):
        await self.ensure(); url=self.upload_domain+'/rest/2.0/pcs/superfile2?'+urllib.parse.urlencode({'method':'upload','app_id':'250528','path':path,'uploadid':uploadid,'partseq':partseq,'access_tokens':self.access}); r=await self.client.post(url,files={'file':('chunk.bin',data,'application/octet-stream')}); r.raise_for_status(); j=r.json();
        if j.get('errno',0)!=0: raise TeraBoxError(j.get('show_msg') or str(j)); return j
    ...
    async def filemeta(self,path,dlink=1):
        ...
        if j.get('errno',0)!=0: raise TeraBoxError(j.get('show_msg') or str(j)); return (j.get('list') or j.get('info') or [None])[0]
    ...
    async def device_code(self):
        if not settings.terabox_client_id: raise TeraBoxError('TERABOX_CLIENT_ID is required for official OAuth')
    ...
    async def connect_tokens(self,access,refresh):
        self.access=access; self.refresh=refresh; await self.token_info(force=True); self._save()
